...
class Element_Pack_Loader {
	...
	public function register_site_scripts() {
		...
		if ( element_pack_is_widget_enabled( 'horizontal-scroller' ) ) {
			wp_register_script( 'scroll-trigger-js', BDTEP_ASSETS_URL . 'vendor/js/ScrollTrigger.min.js', [ 'gsap' ], '3.12.5', true );
		...
		if ( element_pack_is_widget_enabled( 'review-card' )
			or element_pack_is_widget_enabled( 'review-carousel' )
			or element_pack_is_widget_enabled( 'review-grid' ) ) {
			wp_register_script( 'ep-review-card', BDTEP_ASSETS_URL . 'js/modules/ep-review-card' . $suffix . '.js', ['jquery', 'bdt-uikit'], BDTEP_VER, true );
		}
