...
class Hover_Video extends Module_Base {
	...
	protected function render() {
		...
		<div <?php $this->print_render_attribute_string( 'hover_video_attr' ); ?>>
			<span class="hover-video-loader"></span>
			<div <?php $this->print_render_attribute_string( 'hover_video_wrapper' ); ?>>
				<?php
				$i = 0;
				foreach ( $settings['hover_video_list'] as $index => $item ) :
					...
					if ( ! empty( $video_source ) ) {
						$this->add_render_attribute( 'bdt_hover_video_attr', 'src', esc_url( $video_source ), true );
					}

					if ( ! $video_source ) {
						$video_poster = BDTEP_ASSETS_URL . 'images/no-video.svg';
						$this->add_render_attribute( 'bdt_hover_video_attr', 'poster', esc_url( $video_poster ), true );
					}

					?>
					<video <?php $this->print_render_attribute_string( 'bdt_hover_video_attr' ); ?> oncontextmenu="return false;"
						muted>
					</video>
				<?php endforeach; ?>
					...
					foreach ( $settings['hover_video_list'] as $index => $item ) :
						...
						<div class="bdt-hover-bar-wrapper">
							<div class="bdt-hover-bar">
								<div <?php $this->print_render_attribute_string( 'bdt_hover_pro_attr' ); ?>></div>
							</div>
						</div>
					<?php endforeach; ?>
			...
			<div class="bdt-hover-btn-wrapper">
				...
				foreach ( $settings['hover_video_list'] as $index => $item ) :
					...
					if ( ! empty( $item['video_wrapper_link']['url'] ) ) {
						...
						$this->add_render_attribute( 'bdt_hover_btn_attr', 'onclick', "window.open('" . esc_url( $item['video_wrapper_link']['url'] ) . "', '$target')", true );
					}

					?>
					<?php if ( ( $settings['icon_visibility'] == 'yes' or ( ! empty( $item['hover_video_title'] ) ) ) or $settings['btn_progress_visibility'] == 'yes' ) { ?>
						<div <?php $this->print_render_attribute_string( 'bdt_hover_btn_attr' ); ?>>
							...
							<?php if ( ! empty( $item['hover_video_title'] ) ) : ?>
								<div class="bdt-hover-btn-text">
									<?php echo wp_kses_post( $item['hover_video_title'] ); ?>
								</div>
							<?php endif ?>

							<?php if ( $settings['btn_progress_visibility'] == 'yes' ) { ?>
								<div class="bdt-hover-bar">
									<div <?php $this->print_render_attribute_string( 'button_pro_attr' ); ?>></div>
								</div>
							<?php } ?>
