...
class Post_Gallery extends Module_Base {
					...
					'data-hash-settings' => [ 
						wp_json_encode(
							array_filter( [ 
								...
								'activeHash'        => isset( $settings['active_hash'] ),
								'hashTopOffset'     => isset( $settings['hash_top_offset']['size'] ),
								'hashScrollspyTime' => isset( $settings['hash_scrollspy_time']['size'] ),
							] )
						),
					],
				],
			...
			<div data-bdt-dropdown="mode: click;" class="bdt-dropdown bdt-margin-remove-top bdt-margin-remove-bottom bdt-drop">
				<ul class="bdt-nav bdt-dropdown-nav">
					...
					<?php foreach ( $categories as $slug => $category ) { ?>
						<li class="" data-bdt-target="<?php echo esc_attr( trim( $slug ) ); ?>"
	...
	public function render_post() {
		...
		$this->add_render_attribute( 'post-gallery-item', 'data-filter', $this->filter_menu_terms(), true );
	...
	public function filter_menu_terms() {
		$settings    = $this->get_settings_for_display();
		$taxonomy    = isset($settings[ 'taxonomy_' . $settings['posts_source'] ]) ? $settings[ 'taxonomy_' . $settings['posts_source'] ] : '';
