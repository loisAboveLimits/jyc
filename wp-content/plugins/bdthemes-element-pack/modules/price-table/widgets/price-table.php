...
class Price_Table extends Module_Base {
	...
	public function render_image() {
		...
		<div <?php $this->print_render_attribute_string('wrapper'); ?>>
			...
			if (!$thumb_url) {
				printf('<img src="%1$s" alt="%2$s">', esc_url($settings['image']['url']), esc_html($settings['heading']));
			} else {
		...
		if ($settings['heading'] || $settings['sub_heading']) : ?>
			<div <?php $this->print_render_attribute_string('header'); ?>>
				<?php if (!empty($settings['heading'])) : ?>
					<<?php echo esc_attr(Utils::get_valid_html_tag($settings['heading_tag'])); ?> class="bdt-price-table-heading">
						<?php echo esc_html($settings['heading']); ?>
					</<?php echo esc_attr(Utils::get_valid_html_tag($settings['heading_tag'])); ?>>
				<?php endif; ?>
	...
	public function render_price() {
		...
		<div <?php $this->print_render_attribute_string('pricing'); ?>>
			<?php if ($settings['sale'] && !empty($settings['original_price'])) : ?>
				<span class="bdt-price-table-original-price elementor-typo-excluded bdt-display-block" <?php $this->print_render_attribute_string('sale-price-attr'); ?>>
			...
			<?php if (!empty($intpart) || 0 <= $intpart) : ?>
				<span class="bdt-price-table-integer-part" <?php $this->print_render_attribute_string('price-attr'); ?>>
	...
	public function render_features_list() {
		...
		if (!empty($settings['features_list'])) : ?>
			<ul class="bdt-price-table-features-list <?php echo esc_attr($features_hide_on_setup); ?>">
	...
	public function render_features_list_column() {
		...
		if (!empty($settings['features_list'])) : ?>
			<div>
				<ul class="bdt-price-table-features-list bdt-grid-collapse bdt-child-width-expand@m bdt-flex bdt-flex-middle  <?php echo esc_attr($features_hide_on_setup); ?>" data-bdt-grid>
					<?php foreach ($settings['features_list'] as $item) :
	...
	public function render_button() {
		...
		if (class_exists('Easy_Digital_Downloads') && $settings['edd_as_button'] == 'yes') {
