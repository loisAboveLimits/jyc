...
class Review_Card_Grid extends Module_Base {
    ...
    protected function register_controls() {
        ...
        $this->add_responsive_control(
			...
			[ 
				...
				'selectors'            => [ 
					'{{WRAPPER}} .bdt-ep-review-card-grid-item' => '{{VALUE}};',
				],
				'render_type'          => 'template'
			]
		);
        ...
        $this->add_responsive_control(
			...
			[ 
				...
				'selectors' => [ 
					'{{WRAPPER}} .bdt-ep-review-card-grid-item, {{WRAPPER}}.bdt-review-img-inline--yes .bdt-ep-img-inline' => 'grid-gap: {{SIZE}}{{UNIT}};',
				],
			]
		);
    ...
    public function render_reviewer_image($item) {
		...
		$this->add_render_attribute('image-wrap', 'class', 'bdt-ep-review-card-grid-image' . $image_mask, true);
        ?>
    ...
    public function render_review_text($item) {
        ...
        if ( ! $settings['show_review_text'] || empty($item['review_text']) ) {
			return;
		}

		if ( isset( $settings['review_words_length'] ) && ! empty( $settings['review_words_length'] ) ) {
			$this->add_render_attribute( 'review-text', 'class', 'bdt-ep-review-card-grid-text bdt-ep-read-more-text', true );
            ...
            $this->add_render_attribute( 'review-text', 'class', 'bdt-ep-review-card-grid-text', true );
        }
