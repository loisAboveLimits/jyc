...
class Portfolio_Gallery extends Module_Base {
	...
	public function render_thumbnail() {
		...
		if ( ! $thumbnail_html ) {
			printf( '<div class="bdt-gallery-thumbnail"><img src="%1$s" alt="%2$s"></div>', esc_url( $placeholder_img_src ), esc_html( get_the_title() ) );
		} else {
	...
	public function render_filter_menu() {
		...
		<div class="bdt-ep-grid-filters-wrapper" id="<?php echo esc_attr($portfolio_gallery_id); ?>" <?php $this->print_render_attribute_string( 'portfolio-gallery-hash-data' ); ?>>
			...
			<ul id="bdt-ep-grid-filters<?php echo $this->get_id(); ?>" class="bdt-ep-grid-filters bdt-visible@m"
				data-bdt-margin>
	...
	public function render_excerpt() {
		...
		<div class="bdt-portfolio-excerpt">
			...
			if ( has_excerpt() ) {
				...
				echo wp_kses_post(element_pack_custom_excerpt( $this->get_settings_for_display( 'excerpt_limit' ), $strip_shortcode ));
			}
			?>
		</div>
		<?php
						...
						<div class="bdt-flex-inline bdt-gallery-item-link-wrapper">
							...
							<?php if ( ( 'post' == $settings['show_link'] ) || ( 'both' == $settings['show_link'] ) ) : ?>
								...
								<a class="bdt-gallery-item-link<?php echo esc_attr( $link_type_class ); ?>"
									...
									<?php elseif ( 'text' == $settings['link_type'] ) : ?>
										<span>
											<?php esc_html_e( 'VIEW', 'bdthemes-element-pack' ); ?>
										</span>
									<?php endif; ?>
								</a>
							<?php endif; ?>
	...
	public function render_header( $skin = 'default' ) {
		...
		?>
		<div <?php $this->print_render_attribute_string( 'portfolio-wrapper' ); ?>>
			...
			?>
			<div <?php $this->print_render_attribute_string( 'portfolio' ); ?>>
	...
	public function render_post() {
		...
		<div <?php $this->print_render_attribute_string( $element_key ); ?>>
			<div <?php $this->print_render_attribute_string( 'portfolio-item' ); ?>>
				<div <?php $this->print_render_attribute_string( 'portfolio-item-inner' ); ?>>
