...
function uiaddStickyCart(cartFormEl) {
    ...
    var main = jQuery('<div>', {
        class: 'uicore-sticky-add-to-cart elementor-section elementor-section-boxed uicore-main-background'
    });

    // Create the container and add it to the main element
    var container = jQuery('<div>', { class: 'elementor-container' }).appendTo(main);
    ...
    // Clone the cart form and add it to the container
    var cartForm = cartFormEl.clone(true);
    if (cartForm.length) {
        container.append(cartForm);
    }

    // Copy form add to cart text single_add_to_cart_button
    var addToCartText = jQuery('.single_add_to_cart_button').text();
